...
class ProdutoFragment : Fragment() {
    ...
    private fun exibirRecyclerView() {
        ...
        binding.rvListaProduto.layoutManager = StaggeredGridLayoutManager(2,
    ...
    private fun exibirMensagemErro(): Produto?{
        val nomeProduto = binding.etNomeProduto.text.toString()
        val descricaoProduto = binding.etDetalheProduto.text.toString()

        if (nomeProduto.isEmpty() && descricaoProduto.isEmpty()){
            binding.etNomeProduto.error = ERROR_NOME
            binding.etDetalheProduto.error = ERROR_DESCRICAO
